...
void WSI_packet::addTLV(u_int16_t type, u_paraValue_t *data, int len_)
{
	...
	DBG_DUMP("TLV64", &(newTLV->data64()),newTLV->length());
	break;
    case WSI_Unsigned8:
	// Unsigned 8
    ...
    DBG_INFO("Added tlv with type = %04x and lenght = %d",type, newTLV->length());
...
int WSI_packet::serialize(u_int8_t *msg_buf, int buf_len)
{
    ...
    DBG_DUMP("auxHeader", &auxHeader,sizeof(struct wsiHeader));
...
void WSI_packet::parseRxPacket(u_int8_t *msg_buf, int buf_len)
{
    ...
    DBG_INFO("Received packet - Type = %x",header_.nslpID);
...
void WSI_packet::processRxTLV(u_int8_t *msg_buf, int len)
{
	...
	if(DBG_LEVEL > 1) {
	    memset(temp, '\0', 3*MAX_UVALUE_LEN);
	    for(i = 0; i < newTLV->length()+4; i++) {
		sprintf(temp+i*3, "%02X ", msg_buf[i]);
	    }
	}
	auxData = (u_paraValue_t *) (msg_buf + 4);
	DBG_INFO("TLV is of type %x, with length = %d",newTLV->type(),newTLV->length());
...
// 	    break;
	...
	case WSI_UTF8String:
	    ...
	    break;
	default:
	    DBG_ERR("RxTLV: Unknown Parameter Type");
	    break;
	};
...
void WSI_packet::getDataFromTLV(u_int16_t type, void *data, int *len)
{
	...
	case TYPE_UTF8String:
	    DBG_INFO("Length = %d",(int) current->length_);
	...
	case TYPE_Enumerated:
	    ...
	    *len = (int) current->length_;
	    DBG_INFO("Parsing an enumerated tlv - length = %d",*len);
	    switch(*len) {
	    case 1:
		memcpy(data, &(current->u8_data_), current->length_);
		break;
